...
RUN if [ "$MANAGEMENT_ASSET_VERSION" = "latest" ]; then \
  ...
  (echo "Management asset download failed, will use runtime fallback" && touch management.html)
...
RUN CGO_ENABLED=0 GOOS=linux go build -mod=mod -ldflags="-s -w -X 'main.Version=${VERSION}-plus' -X 'main.Commit=${COMMIT}' -X 'main.BuildDate=${BUILD_DATE}'" -o ./CLIProxyAPIPlus ./cmd/server/
...
COPY --from=management_asset_downloader /tmp/management.html /tmp/management.html
