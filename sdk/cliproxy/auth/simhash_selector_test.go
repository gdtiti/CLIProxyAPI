...
func TestSimHashSelectorPrefersColdStartAuthsUntilPoolFills(t *testing.T) {
	...
	first, err := selector.Pick(context.Background(), "codex", "gpt-5.4", cliproxyexecutor.Options{}, auths)
	if err != nil {
		t.Fatalf("first pick error: %v", err)
	}
	second, err := selector.Pick(context.Background(), "codex", "gpt-5.4", cliproxyexecutor.Options{}, auths)
	if err != nil {
...
func TestSimHashSelectorPoolOnlyAdmitsOneNewAuthAfterFilled(t *testing.T) {
	...
	if selected.ID != "c" {
		t.Fatalf("selected %q, want existing pool member c while outsider admission is cooling down", selected.ID)
	}
}
