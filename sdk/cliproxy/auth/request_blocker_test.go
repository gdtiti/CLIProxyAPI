...
func TestRequestBodyHashStableAcrossJSONKeyOrder(t *testing.T) {
	...
	if _, ok := requestBodyAnalysisFromMetadata(leftOpts.Metadata); !ok {
		t.Fatal("expected left request body analysis metadata to be populated")
	}
	if _, ok := requestBodyAnalysisFromMetadata(rightOpts.Metadata); !ok {
		t.Fatal("expected right request body analysis metadata to be populated")
	}
}
...
func TestIsBlockableInvalidRequestError(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "model support error excluded", err: &Error{HTTPStatus: http.StatusUnprocessableEntity, Message: "requested model is not supported"}, want: false},
