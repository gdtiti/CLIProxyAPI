...
// SetConfig hot-updates the selector configuration without clearing runtime pool state.
...
func (s *SimHashSelector) Pick(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*Auth) (*Auth, error) {
	...
	if len(s.pool.members) < s.effectivePoolSizeLocked() && len(outsiders) > 0 {
		if !s.pool.everFilled || s.pool.lastAdmittedAt.IsZero() || now.Sub(s.pool.lastAdmittedAt) >= s.admitCooldownLocked() {
			admitted := s.pickAdmissionCandidateLocked(now, outsiders)
			if admitted != nil {
				alreadyFilled := s.pool.everFilled
	...
	return s.pickBestCandidate(opts, candidates), nil
}

func (s *SimHashSelector) pickBestCandidate(opts cliproxyexecutor.Options, auths []*Auth) *Auth {
	if len(auths) == 0 {
		return nil
	}
	if len(auths) == 1 {
